...
subject_ids=np.loadtxt(os.path.join(subject_dir, subject_id_filename),dtype='str',ndmin=1)
control_dir=args.control_dir
hemis=['lh','rh']
cortex=nb.freesurfer.io.read_label(os.path.join(subject_dir,'fsaverage_sym/label/lh.cortex.label'))
...
for fs_id in subject_ids:
    ...
    if os.path.isfile(os.path.join(subject_dir, fs_id, 'xhemi/surf_meld/lh.on_lh.intra_z.gm_FLAIR_0.75.sm10.mgh')):
        ...
        measures=['.on_lh.intra_z.thickness.sm10.mgh', '.asym.on_lh.intra_z.thickness.sm10.mgh',
              ...
              '.on_lh.curv.mgh','.on_lh.sulc.mgh',
                  '.asym.on_lh.curv.mgh','.asym.on_lh.sulc.mgh']
    demo=nb.load(os.path.join(subject_dir, fs_id, 'xhemi/surf_meld', hemis[0]+measures[0]))
    for h in hemis:
        for m in measures:
            control_mu=io.load_mgh(os.path.join(control_dir,f,h+'.mu'+m))
            control_std=io.load_mgh(os.path.join(control_dir,f,h+'.std'+m))
            subject_measure=io.load_mgh(os.path.join(subject_dir, fs_id, 'xhemi/surf_meld', h+m))
            z_measure=np.divide((subject_measure-control_mu),control_std,out=np.zeros_like(subject_measure), where=control_std!=0)
            io.save_mgh(os.path.join(subject_dir, fs_id, 'xhemi/surf_meld', h+'.inter_z'+m),z_measure,demo)
